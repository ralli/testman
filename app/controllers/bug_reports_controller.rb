...
class BugReportsController < ApplicationController
  def new
    ...
    if params[:teststeprun_id].blank?
      ...
      title = I18n::t('controller.bug_reports.default_title_with_details', :testsuiteid => testsuite.id, :teststep_no => teststeprun.teststep.position, :testcase_no => testcaserun.position)
      testcase_url = url_for(testcaserun.testcase)
      message = I18n::t('controller.bug_reports.default_message_with_details', :testsuiteid => testsuite.id, :url => url, :testcase_url => testcase_url, :teststep_no => teststeprun.teststep.position, :testcase_no => testcaserun.position)
    end
    @bug_report = BugReport.new(:title => title, :message => message, :testcaserun_id => params[:testcaserun_id], :teststeprun_id => params[:teststeprun_id] )
  end
